...
func run() -> void:
    ...
    var counts:={"static_bodies":0,"concave":0,"boxes":0,"cylinders":0}
    ...
    print("PERFORMANCE METRICS build_ms=",Time.get_ticks_msec()-started," static_bodies=",counts.static_bodies," concave=",counts.concave," boxes=",counts.boxes," cylinders=",counts.cylinders)
    check(get_nodes_in_group("static_render_cells").is_empty(),"Static batching does not add scene-wide distance culling")
