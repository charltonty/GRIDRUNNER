...
Standalone play starts maximized. The 1280×720 setting is the virtual UI design canvas and scales to the actual window. Godot's embedded game view controls its own size: choose **Stretch to Fit** in the Game view menu, or disable **Embed Game on Next Play**, to test the real standalone window.
...
The project was validated on Windows with Godot 4.4.1. The performance suite guards primitive collision policy and prevents scene-wide distance culling from returning unnoticed.
