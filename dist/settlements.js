...
export function makeSettlements(scene,solids){const groups=[],actors=[],lamps=[];const material=new Map();const mat=(c,glow=false)=>{const key=c+':'+glow;if(!material.has(key))material.set(key,new T.MeshStandardMaterial({color:c,roughness:.86,metalness:.12,emissive:glow?c:0,emissiveIntensity:glow?.8:0}));return material.get(key);};
 ...
 for(let j=0;j<2;j++){const x=j*22-11,h=site.style==='garage'?6:4.5;box(x,.12,-7,18,.24,16,0x55554e);box(x,h/2,-15,18,h,.45,wall,true);box(x-9,h/2,-7,.4,h,16,wall,true);box(x+9,h/2,-7,.4,h,16,wall,true);box(x,h+.12,-7,19,.35,17,0x343e40);box(x,h-.35,1,18,.65,.4,trim);box(x,h-1.1,.85,4,.18,.2,glow,false,true);
 ...
 box(x+4,h+.7,-9,1.7,1,1.7,0x49555a);box(x,h-.9,3,18,.15,4,0x5c6256);for(const dx of [-8,8])box(x+dx,(h-1)/2,4.7,.15,h-1,.15,trim);
 }
 ...
 return {groups,actors,update(s,time,quality){for(const lamp of lamps)lamp.visible=quality!=='LOW';for(const {site,g}of groups)g.visible=site.leg===s.leg&&Math.hypot(site.x-s.pos.x,site.z-s.pos.z)<550;for(const {actor,site}of actors){actor.rotation.y=Math.atan2(s.pos.x-site.x+8,s.pos.z-site.z-4);actor.position.y=Math.sin(time*1.3)*.025;}},near(s){return SETTLEMENTS.filter(p=>p.leg===s.leg).map(p=>({...p,d:Math.hypot(s.pos.x-p.x,s.pos.z-p.z)})).sort((a,b)=>a.d-b.d)[0];}};
}
