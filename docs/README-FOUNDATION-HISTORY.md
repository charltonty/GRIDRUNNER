...
Use **Godot 4.4.1** (the validated engine baseline). Import `project.godot` from the repository root. Press **F5**, then New Expedition. The project uses Compatibility rendering.
...
4. Select `project.godot` from the repository root.
