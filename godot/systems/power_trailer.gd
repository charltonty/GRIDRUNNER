    ...
    match active_source:
        ...
        "FUEL":
            if fuel_liters > 0.0:
                ...
                var generated := input_kw * delta / 3600.0
                var burn := generated / generator_kwh_per_liter
                fuel_liters = max(0.0, fuel_liters - burn)
            else:
                active_source = "OFF"
    battery_kwh = min(battery_capacity_kwh, battery_kwh + input_kw * delta / 3600.0)

func transfer_to(target: EnergySystem, power_kw: float, delta: float) -> float:
    var available := min(power_kw * delta / 3600.0, battery_kwh)
    var accepted := target.charge(available * 3600.0 / max(delta, 0.001), delta)
