...
func generate(delta: float, battery: EnergySystem) -> float:
    if not running or fuel_liters <= 0.0:
        stop()
        return 0.0
    var possible_kwh: float = rated_output_kw * delta / 3600.0
    var fuel_needed: float = possible_kwh / usable_kwh_per_liter()
    var actual_fuel: float = minf(fuel_needed, fuel_liters)
    var available_kwh: float = actual_fuel * usable_kwh_per_liter()
    var accepted_kwh: float = battery.charge(available_kwh * 3600.0 / maxf(delta, 0.001), delta)
